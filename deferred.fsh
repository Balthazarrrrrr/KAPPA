...
vec4 cloudSystem(vec3 Direction, float vDotL, float dither, float lightNoise, vec3 SkyboxColor, out float skylightIntensity) {
    ...
    if (visibleVol || within > 0.0) {
        ...
        const mat2x3 PhaseAsymmetry = mat2x3(
            vec3(0.5, 0.35, 0.9),
            vec3(0.5, 0.35, 0.9)
        );

        float FirstEventPhase = cloudPhaseKN(vDotL, vec2(2800.0, 0.35));
        ...
        for (uint i = 0; i < steps; ++i, rPos += rStep) {
            ...
            vec3 sunlight   = ReadSunlightGradient(rPos, Direction);
                ...
                for (uint j = 0; j < 8; ++j) {
                    ...
                    vec3 asymmetry      = pow(PhaseAsymmetry[0], vec3((1.0 + (lightOD + density * rLength) * td)));
                    vec3 asymmetrySky   = pow(PhaseAsymmetry[1], vec3((1.0 + (skyOD + density * rLength) * td)));

                    stepScatter += scattering * vec3(j == 0 ? FirstEventPhase : cloudPhaseNew(vDotL, asymmetry), cloudPhaseSky(Direction.y, asymmetrySky), cloudPhaseSky(-Direction.y, asymmetry)) * p;
                }
        ...
        if (Distance < CLOUD_PLANE1_CLIP) {
            ...
            if (Density > 0.0) {
                ...
                if (maxOf(atmosFade) < 1e-4) {
                    ...
                    vec3 phaseG         = pow(PhaseAsymmetry[0], vec3(1.0 + (lightOD + Density * RLength) * SigmaT) * vec3(1, 1, 1));
                    vec3 phaseGSky      = pow(PhaseAsymmetry[1], vec3(1.0 + (skyOD + Density * RLength) * SigmaT));

                    float scatterScale  = pow(1.0 + 1.0 * (lightOD) * SigmaT, -1.0 / 1.0) * bounceEstimate;
                    float SkyScatterScale = pow(1.0 + 1.0 * skyOD * SigmaT, -1.0 / 1.0) * bounceEstimate;

                    scattering.x  += baseScatter * cloudPhaseNewKN(vDotL, phaseG) * scatterScale;
        ...
        if (Distance < CLOUD_PLANE0_CLIP) {
            ...
            if (Density > 0.0) {
                ...
                if (maxOf(atmosFade) < 1e-4) {
                    ...
                    vec3 phaseG         = pow(PhaseAsymmetry[0], vec3(1.0 + (lightOD + Density * RLength) * SigmaT) * vec3(1 / sqrt2));
                    vec3 phaseGSky      = pow(PhaseAsymmetry[1], vec3(1.0 + (skyOD + Density * RLength) * SigmaT));
